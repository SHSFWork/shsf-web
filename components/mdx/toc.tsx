...
import { TocType } from "@shsfwork/lib/toc";
import { ScrollArea } from "@shsfwork/components/shadcn/scroll-area";
...
export default function Toc({ toc }: TocProps) {
  ...
  return (
    <div className="py-2 border-y">
      <h6 className="font-semibold text-lg">Table of Contents</h6>
      <nav className="">
        <ScrollArea>
...
function Tree({ tree, level = 1, activeItem }: TreeProps) {
  return tree?.items?.length && level < 3 ? (
    <ul className={cn("m-0 list-none max-h-96", { "pl-2": level !== 1 })}>
      {tree.items.map((item, index) => {
        return (
          <li key={index} className={cn("mt-0 pt-0.5")}>
            <Link
              title={item.title}
              href={item.url}
              className={cn(
